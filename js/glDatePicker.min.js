		...
		glDatePicker.prototype =
		{
			...
			render: function(renderCalback) {
				...
				var onResize = function() {
					...
					calendar.css(
					{
						top: (el.parents('.date').position().top + el.parents('.right-about').position().top + 42) + 'px', //elPos.top + el.outerHeight() + options.calendarOffset.y 
						left: (el.parents('.right-about').position().left + 25) + 'px' //elPos.left + options.calendarOffset.x
					});
				};
				...
				for(var row = 0, cellIndex = 0; row < maxRow + 1; row++) {
					for(var col = 0; col < maxCol; col++, cellIndex++) {
						...
						if(!row) {
							...
							cell.html('<span class="gldp-date">' + cellDateVal.date + '</span>');
